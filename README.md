...
4. **建立连接** — 使用 Auth Key 登录 Tailscale 网络，获取虚拟 IP
5. **开启子网路由和出口节点** — 认证完成后，自动检测子网并开启 Subnet Router 和 Exit Node
6. **验证连通性** — 测试 3389 端口 TCP 连接是否正常
7. **保持运行** — 持续输出连接信息并保持 Runner 活跃，直到手动取消
